...
button_finances_all = KeyboardButton(text="Потрачено всего")
...
@dp.message(F.text == "Потрачено всего")
async def total_expenses(message: Message):
    telegram_id = message.from_user.id
    cursor.execute(
        '''SELECT category1, expenses1, category2, expenses2, category3, expenses3 FROM users WHERE telegram_id = ?''',
        (telegram_id,))
    user_data = cursor.fetchone()

    if user_data:
        category1, expenses1, category2, expenses2, category3, expenses3 = user_data

        report = (f"Вы потратили всего:\n"
                  f"{category1} - {expenses1:.2f} RUB,\n"
                  f"{category2} - {expenses2:.2f} RUB,\n"
                  f"{category3} - {expenses3:.2f} RUB")

        await message.answer(report)
    else:
        await message.answer("Вы еще не вводили данные о своих расходах.")
